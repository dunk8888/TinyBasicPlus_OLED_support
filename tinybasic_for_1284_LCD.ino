...
// v0.15a: 02/02/2015
//        40x4 character LCD (which has two Enable terminal, E1, and E2) support is added.
//        Hopefully it will also work on ATMega328P, but I coded and checked only on ATMega1284P.
//        (RAM (whose area is available for BASIC) of 328P is just 2kB, ATMega1284 is 16kB.)
//        For fast display, I used, "LiquidCrystalFast" library on the web.
//        This program is a mixed up version of Ben Heck's Retro Basic Computer's source        
//        and AVR Basic computer. As a result of this composite, this code enables
...
static const unsigned char initmsg[]          PROGMEM = "TinyBasic Plus v0.14 supporting 40x4 LCD";
static const unsigned char memorymsg[]        PROGMEM = " bytes free. ";
#ifdef ARDUINO
#ifdef ENABLE_EEPROM
static const unsigned char eeprommsg[]        PROGMEM = " EEProm bytes tot.";
...
void printmsg2(const unsigned char *msg)
{
  printmsgNoNL(msg);
}
...
static void lcdChar(byte c) {
	...
	if (c == 8) {	//Backspace?
		...
		if (cursorX > 0) {
			...
			screenMem[120 + cursorX] = 32;	//Erase it from memory
			doFrame(120 + cursorX); //Redraw screen up to that amount
	...
	if (cursorX == 40 or c == 10) {			//Did we hit Enter or go type past the end of a visible line?
	
		for (int xg = 0 ; xg < 40 ; xg++) {

//  //                    1gyo = 3gyo
//			screenMem[0 + xg] = screenMem[80 + xg];
// ///                    3gyo = 2gyo
//			screenMem[80 + xg] = screenMem[40 + xg];
/////                     2gyo = 4 gyo	
//			screenMem[40 + xg] = screenMem[120 + xg];

  //                    1gyo = 3gyo
			screenMem[0 + xg] = screenMem[40 + xg];
 ///                    3gyo = 2gyo
			screenMem[40 + xg] = screenMem[80 + xg];
///                     2gyo = 4 gyo	
			screenMem[80 + xg] = screenMem[120 + xg];



                       //4gyo = null
//                        screenMem[0 + xg] = screenMem[40 + xg];
//                        screenMem[40 + xg] = screenMem[100 + xg];
//                        screenMem[100 + xg] = screenMem[140 + xg];
			screenMem[120 + xg] = 32;
//			screenMem[0 + xg] = screenMem[40 + xg];
